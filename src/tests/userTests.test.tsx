...
const server = setupServer(
  http.post("http://127.0.0.1:5000/search", () => {
  ...
  http.post("http://127.0.0.1:5000/playlist", () => {
...
test("searched song is played", async () => {
  ...
  fireEvent.click(
    screen.getByRole("button", { name: /nameExample artistExample/i })
  );
...
test("newly created playlist is sent to spotify", async () => {
  ...
  const songOne = screen.getByRole("button", {
    name: /nameExample artistExample/i,
  });

  const addToPlaylistBtn = within(songOne).getByRole("button", {
  ...
  // add another song
  const songTwo = screen.getByRole("button", {
    name: /secondName secondArtist/i,
  });
  expect(songTwo).toBeVisible();

  const addToPlaylistBtnSong2 = within(songTwo).getByRole("button", {
    name: /add to playlist/i,
  });
  ...
  const removeFromPlaylistBtn = within(
    within(playlistContainer).getAllByTestId("playlist-song")[1]
  ).getByRole("button", { name: /remove from playlist/i });
  ...
  const addToSpotifyButton = screen.getByRole("button", {
    name: /add to spotify/i,
  });
  fireEvent.click(addToSpotifyButton);

  await waitFor(() =>
    expect(screen.getByText(/Playlist Created/i)).toBeVisible()
  );
  ...
  await waitFor(() =>
    expect(screen.queryByText(/Playlist Created/i)).toBeNull()
  );
});
...
test("newly created playlist cleared", async () => {
  ...
  const clearPlaylistButton = screen.getByRole("button", {
    name: /clear playlist/i,
  });
